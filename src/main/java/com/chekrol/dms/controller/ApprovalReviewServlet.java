...
public class ApprovalReviewServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.setAttribute("pageTitle", "Review " + document.getDocumentCode());
            request.getRequestDispatcher("/WEB-INF/views/approvals/review.jsp")
                    .forward(request, response);
        } catch (NumberFormatException exception) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Invalid document ID.");
        } catch (Exception exception) {
            throw new ServletException("Unable to load the approval review page.", exception);
        }
    }
}
