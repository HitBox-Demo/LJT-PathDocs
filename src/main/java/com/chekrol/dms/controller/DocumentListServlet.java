...
public class DocumentListServlet extends HttpServlet {
    private static final Set<String> ALLOWED_VIEWS = Set.of(
            "repository", "draft", "pending", "returned", "department", "search"
    );
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String view = ALLOWED_VIEWS.contains(requestedView) ? requestedView : "repository";
        ...
        try {
            request.setAttribute(
                    ...
                    AppConfig.isDemoMode()
                            ? DemoData.listDocuments(currentUser, view, query)
                            : documentDAO.listForUser(currentUser, view, query)
            );
            request.setAttribute("view", view);
            request.setAttribute("query", query);
            request.setAttribute("pageTitle", pageTitle(view, currentUser));
            request.getRequestDispatcher("/WEB-INF/views/documents/list.jsp")
                    .forward(request, response);
        } catch (Exception exception) {
            throw new ServletException("Unable to load documents.", exception);
        }
    }
    ...
    private String pageTitle(String view, User user) {
        return switch (view) {
            ...
            case "department" -> (user.getDepartmentName() == null
                    ? "Department"
                    : user.getDepartmentName()) + " Folder";
