...
public class AuthenticationService {
    ...
    public User authenticate(String username,String password)throws SQLException{
        if(AppConfig.isDemoMode())return DemoData.authenticate(username,password);
        User user=userDAO.findByUsername(username);
        if(user==null||!"ACTIVE".equals(user.getStatus())||!PasswordUtil.verify(password,user.getPasswordHash()))return null;
        return user;
    }
}
